...
导入语句应该以包/模块的名称的字母顺序进行排序（相对导入在绝对导入之前）。
...
明确声明要把什么带入作用域的导入应该被分组为：模块、常量、类型、宏和函数。

这些分组应该按照该顺序指定，每组的内容应该按照字母顺序排序，通常模块的导入在单独的一行。
...
通常情况下，包含模块定义的文件不应包含任何在该模块之外运行的其他代码。
也就是说，应该在文件的顶部用 `module` 关键字声明模块，在文件结尾用`end` 结束声明。
前后都不要放其他代码（除了模块前的docstring）。
在这种情况下，模块中的代码**不应该**缩进。

有时，例如为了测试，或者为了命名一个枚举时，*最好*在文件的中间声明一个子模块。
在这种情况下，子模块中的代码**应该**缩进。
